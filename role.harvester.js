    ...
    run: function(creep) {
        ...
        for (var i=0;i<sources.length;i++) {
          ...
          if (source.id === creep.memory.pref && (source.miners.includes(creep.id)==false) && (creep.id != null)) {
            ...
            creep.room.memory.allSources[i].miners.push(creep.id);
        ...
        var link = sourceObj.pos.findInRange(FIND_MY_STRUCTURES,5,{ filter: (structure) => {return (structure.structureType == STRUCTURE_LINK && structure.energy<structure.energyCapacity )  } });
