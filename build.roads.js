var buildRoads = {
    ...
    getCallbackDoubleroad: function(roomName) {
      ...
      room.find(FIND_STRUCTURES).forEach(function(structure) {
        if (structure.structureType == STRUCTURE_WALL) {
          costs.set(structure.pos.x, structure.pos.y, 255);
          costs.set(structure.pos.x+1, structure.pos.y, Math.max(255,costs.get(structure.pos.x+1, structure.pos.y)));
          costs.set(structure.pos.x, structure.pos.y+1, Math.max(255,costs.get(structure.pos.x, structure.pos.y+1)));
        }
      });
      return costs;
    },
