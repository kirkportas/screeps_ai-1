...
var mainSpawn = {
  ...
  global.spawnArmy = function(spawn) {
    if (_.filter(Game.creeps, (creep)  => creep.memory.manual == '1').length<6) {
    ...
    if (_.filter(Game.creeps, (creep)  => creep.memory.manual == '3').length<4) {
      if (createCreepAdvanced(spawn,'attacker',createBody({tough:8,move:8,attack:8}),{flag:'attack',manual:'3'})) return true;
    }
    return false;
  }
