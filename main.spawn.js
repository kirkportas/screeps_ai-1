    ...
    global.spawnHarvesters = function(spawn) {
      ...
      for (var i=0;i<sources.length;i++) {
        ...
        if (energyAvav>=750) {
          ...
          if ((source.miners.length<1 || (source.miners.length==1 && Game.getObjectById(source.miners[0]).ticksToLive<100) && source.safe && count.harvesters>0)) {
      ...
      for (var roomName in scout) {
        if (scout[roomName].danger==0) {
          ...
          for (var sourceId in sources) {
            ...
            var sourcePerTick=(carrycap/((pathLen*2)+25+2)*0.6); //Empirisk verdi for å justere feil
